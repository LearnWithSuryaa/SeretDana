<template>
  <div class="space-y-6 pb-10">
    <!-- Heading -->
    <h2 class="text-2xl font-bold text-gray-800">Anggaran & Analisis</h2>
    ...
    >
      ...
      <div class="flex justify-between items-center mb-6">
        <h3 class="text-lg font-semibold text-gray-700">Ringkasan Bulan Ini</h3>
        <button
          @click="openAddBudgetModal(null)"
      ...
      <!-- Budget List -->
      <div v-if="monthlyBudgets?.length" class="space-y-4">
        <div
          v-for="budget in monthlyBudgets"
          :key="budget.category_id"
          class="bg-white/70 backdrop-blur p-4 rounded-xl border border-gray-200 shadow-sm hover:shadow-md transition"
        >
          <!-- Header -->
          ...
          <div class="w-full bg-gray-200 rounded-full h-2 overflow-hidden">
            <div
              ...
              :style="{
                width:
                  Math.min(100, (budget.spent / budget.limit_amount) * 100) +
                  '%',
              }"
            ></div>
          </div>
          ...
          <div class="flex justify-between items-center mt-2">
            ...
            <button
              @click="openAddBudgetModal(budget)"
      ...
      <p v-else class="text-center text-gray-400 italic py-8">
        Belum ada anggaran yang diatur bulan ini.
      </p>
      ...
      <div class="mt-8 pt-6 border-t border-gray-100">
        ...
        <div
          v-if="totalSpentForChart > 0"
          class="w-full h-64 bg-white/70 backdrop-blur rounded-xl shadow-md flex items-center justify-center p-4"
        >
          <canvas ref="expenseChartCanvas"></canvas>
        </div>
        <div
          v-else
          class="w-full h-64 bg-gray-100 rounded-xl flex items-center justify-center text-gray-400 border border-dashed border-gray-300"
        >
          <p>Tidak ada data pengeluaran untuk grafik.</p>
        </div>
      </div>
    </div>
...
const props = defineProps({
  ...
  categories: {
    // This is the prop from DashboardPage
    type: Array,
    default: () => [],
  },
});
...
const getCategoryName = (categoryId) => {
  ...
  if (!category) {
    console.warn(
      `Category with ID ${categoryId} not found in props.categories.`
    );
  }
  ...
  props.transactions
    ...
    .forEach((t) => {
      const categoryName = t.category_id
        ...
        : "Tidak Dikategorikan";
      console.log(
        `  Transaction: ${t.description}, Amount: ${t.amount}, Category ID: ${t.category_id}, Resolved Category: ${categoryName}`
      );
